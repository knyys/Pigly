    ...
    <div class="weight-log__content">
        ...
        <table class="weight-log-table">
            ...
            <tr>
                ...
                <td><a class="weight-log__detail-btn" href="#modal-{{$data->id}}">✐</a></td>
            </tr>
            @endforeach
        </table>
            ...
            <form class="modal__detail-form" action="{{ route('weight_logs.store') }}" method="post">
                ...
                <div class="modal-form__group">
                    ...
                    <div class="modal-form__input">
                        <input type="date" name="date" id="date-new" value="{{ old('date', '') }}" />
                    </div>
                ...
                <div class="modal-form__group">
                    ...
                    <div class="modal-form__input">
                        <input type="text" name="weight" id="weight-new" value="{{ old('weight') }}"/>
                ...
                <div class="modal-form__group">
                    ...
                    <div class="modal-form__input">
                        <input type="text" name="calories" id="calories-new" value="{{ old('calories', '') }}" />
                ...
                <div class="modal-form__group">
                    ...
                    <div class="modal-form__input">
                        <input type="text" name="exercise_time" id="exercise_time-new" value="{{ old('exercise_time', '') }}" />
                    </div>
                    <div class="form__error">
                        @error('exercise_time') 
                        {{ $message }}
                ...
                <div class="modal-form__group">
                    ...
                    <div class="modal-form__input">
                        <textarea name="exercise_content" id="exercise_content-new">{{ old('exercise_content', '') }}</textarea>
                    </div>
                    <div class="form__error">
...
<div class="modal-form__input">
<input type="date" name="date" id="date-{{ $data->id }}"
...
<div class="modal-form__input">
<input type="number" name="weight" id="weight-{{$data->id}}" value="{{ old('weight', $data->weight) }}" />
<span class="input-unit">kg</span>
</div>
<div class="form__error">
@error('weight')
...
<div class="modal-form__input">
<input type="number" name="calories" id="calories-{{$data->id}}" value="{{ old('calories', $data->calories) }}" />
<span class="input-unit">cal</span>
</div>
<div class="form__error">
@error('calories')
...
<div class="modal-form__input">
<input type="text" name="exercise_time" id="exercise_time-{{$data->id}}" value="{{ old('exercise_time', $data->exercise_time) }}" />
</div>
<div class="form__error">
@error('exercise_time')
{{ $message }}
...
<div class="modal-form__input">
<textarea name="exercise_content" id="exercise_content-{{$data->id}}">{{ old('exercise_content', $data->exercise_content) }}</textarea>
</div>
<div class="form__error">
@error('exercise_content')
{{ $message }}
...
$editId = null;
foreach ($datas as $data) {
// ここは edit-{id} のエラーバッグを判定する
if ($errors->getBag('edit-' . $data->id)->any()) {
$editId = $data->id;
break;
}
}
...
@if($editId)
location.hash = "#modal-{{ $editId }}";
@else
location.hash = "#modal-new";
@endif
}
</script>
@endif
...
document.querySelectorAll('.btn__return').forEach(button => {
button.addEventListener('click', function(e) {
e.preventDefault();
// ハッシュを消す
history.pushState("", document.title, window.location.pathname + window.location.search);
// ページをリロードしてバリデーションエラーをクリア
location.reload();
});
});
});
